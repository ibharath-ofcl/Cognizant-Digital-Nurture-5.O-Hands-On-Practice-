...
public class EmployeeManagementSystemApplication {
	...
	@Bean
	public CommandLineRunner demo(EmployeeRepository employeeRepository, DepartmentRepository departmentRepository, ProjectRepository projectRepository) {
		return args -> {
			if (departmentRepository.count() > 0 || projectRepository.count() > 0) {
			...
			// Save projects (Secondary Data Source)
			Project alpha = Project.builder().name("Project Alpha").budget(100000.0).build();
			Project beta = Project.builder().name("Project Beta").budget(250000.0).build();
			...
			// 14. Project (Secondary Data Source): findByNameContainingIgnoreCase
			System.out.println("\n10. Project (Secondary DB) findByNameContainingIgnoreCase('alpha'):");
			projectRepository.findByNameContainingIgnoreCase("alpha")
					.forEach(proj -> System.out.println("- Found Project: " + proj.getName() + " with budget $" + proj.getBudget()));
