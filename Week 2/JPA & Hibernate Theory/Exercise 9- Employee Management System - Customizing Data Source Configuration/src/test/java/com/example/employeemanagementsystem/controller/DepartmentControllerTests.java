...
class DepartmentControllerTests {
    ...
    @Test
    void testGetDepartmentProjections() throws Exception {
        Department dept = departmentRepository.save(Department.builder().name("Engineering").build());
        employeeRepository.save(Employee.builder()
                .name("Alice")
                .email("alice@example.com")
        ...
        mockMvc.perform(get("/api/departments/projections").param("name", "Engineering"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
                .andExpect(jsonPath("$[0].name", is("Engineering")))
                .andExpect(jsonPath("$[0].employeeCount", is(1)));
    }
}
